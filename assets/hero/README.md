...
# MP4 -> palette-optimized 1200x675 looping GIF
ffmpeg -i final.mp4 \
  -vf "fps=15,scale=1200:-1:flags=lanczos,palettegen=max_colors=128:stats_mode=diff" \
  -y palette.png
ffmpeg -i final.mp4 -i palette.png \
  -lavfi "fps=15,scale=1200:-1:flags=lanczos[x];[x][1:v]paletteuse=dither=bayer:bayer_scale=3:diff_mode=rectangle" \
  -loop 0 -y ../pi-gateway-hero.gif
```

## Composition notes

- 1920x1080 design, 8s, fades from/to black for a seamless loop.
