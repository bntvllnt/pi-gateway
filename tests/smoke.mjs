...
async function testExtensionRegistration() {
  ...
  assert.equal(
    typeof extension,
    "function",
    "index.ts default export must be a function",
  );
  ...
  assert.match(tools.get("gateway_status").description, /Read-only/);
  assert.match(tools.get("gateway_start").description, /Side effect: starts/);
  assert.match(tools.get("gateway_stop").description, /Side effect: stops/);
  assert.ok(events.has("session_start"), "expected session_start handler");
  assert.ok(
    events.has("session_shutdown"),
    "expected session_shutdown handler",
  );

  const source = readFileSync(path.join(packageRoot, "index.ts"), "utf8");
  assert.match(source, /const LOG_PATH =/);
  assert.match(source, /const MAX_PROBE_BODY_BYTES =/);
  assert.match(source, /function buildDaemonEnv/);
...
async function testServerBindAndHealthz() {
  ...
  try {
    ...
    const modelsRes = await fetch(
      `http://127.0.0.1:${handle.address.port}/v1/models`,
    );
    ...
    assert.equal(
      modelsBody.object,
      "list",
      "models payload object must be 'list'",
    );
    ...
    log(
      `PASS server bind + healthz + /v1/models (${modelsBody.data.length} models)`,
    );
  } finally {
